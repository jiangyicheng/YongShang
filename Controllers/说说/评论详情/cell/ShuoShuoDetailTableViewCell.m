...
@interface ShuoShuoDetailTableViewCell ()
{
...
@implementation ShuoShuoDetailTableViewCell

- (void)awakeFromNib {
    self.contentLab.font = DEF_FontSize_12;
    self.contentLab.textColor = getColor(@"595959");
    self.companyAndNameLab.font = DEF_FontSize_12;
    self.companyAndNameLab.text = @"(姜易成)阿里UFO";
    self.companyAndNameLab.textColor = getColor(@"595959");
    
    UITapGestureRecognizer* singletap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(iconViewClick)];
    ...
    self.companyAndNameLab.sd_layout
...
{
    _model = model;
    _companyAndNameLab.text = [NSString stringWithFormat:@"(%@)%@",model.linkman,model.companyname];
...
-(void)iconViewClick
{
    if ([self.delegate respondsToSelector:@selector(iconViewDidClick:)]) {
        [self.delegate iconViewDidClick:self];
    }
}
