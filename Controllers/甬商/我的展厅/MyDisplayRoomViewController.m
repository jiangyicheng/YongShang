...
@interface MyDisplayRoomViewController ()<UIScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIGestureRecognizerDelegate,JYCDropDownMenuDelegate,TZImagePickerControllerDelegate,UITextViewDelegate>
{
    ...
    NSMutableArray* _bigImageUrlArray;
    ...
    NSString* _smallPicUrlString;          //所有缩略图路径
    NSString* _bigPicUrlString;            //所有大图路径
...
@implementation MyDisplayRoomViewController
...
{
    [YSBLL upLoadThePictureWithFlagId:_enterPriseNum andProducturl1:_bigPicUrlString ? _bigPicUrlString : @"" andProducturl1s:_smallPicUrlString ? _smallPicUrlString : @"" andMemo:_memoString ? _memoString :@"" andResultBlock:^(CommonModel *model, NSError *error) {
        if ([model.ecode isEqual:@"0"]) {
...
-(void)fileUploadImage
{
    NSDictionary* dict = @{@"flagId":_enterPriseNum,
                           @"producturl1":_bigPicUrlString?_bigPicUrlString : @"",
                           @"producturl1s":_smallPicUrlString ? _smallPicUrlString : @""};
    [[AFAppDotNetAPIClient sharedClient]POST:@"" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
-(void)loadEnterPriseDetailData
{
    ...
    _bigImageUrlArray = [[NSMutableArray alloc]init];
    if (self.model) {
        if (![self.model.producturl1s isEqualToString:@""]) {
            NSString* tempStr = [self.model.producturl1s substringWithRange:NSMakeRange(0, 1)];
            NSString* smallImageStr = self.model.producturl1s;
            NSString* bigImageStr = self.model.producturl1;
            //如果第一个字符是逗号的话就删除逗号
            ...
            NSArray* arrs = [smallImageStr componentsSeparatedByString:@","];
            NSArray* arr = [bigImageStr componentsSeparatedByString:@","];
            _imageUrlArray = [arrs mutableCopy];
            _bigImageUrlArray = [arr mutableCopy];
        }
    }
}
...
//
//    [_selectedPhoto addObject:[UIImage imageNamed:@"haoyouqiye"]];
...
-(void)changeOrginImageToBase64
{
    ...
    _bigPicUrlString  = [NSString string];
    _smallPicUrlString = [NSString string];
    for (int i = 0 ; i < 6; i++) {
        if (i < _imageUrlArray.count) {
            if ([_imageUrlArray[i] isKindOfClass:[UIImage class]]) {
                NSString* imageStr =  [self imageChangeToBase64:_imageUrlArray[i]];
                _memoString = [_memoString stringByAppendingString:[NSString stringWithFormat:@",%@",imageStr]];
            }else{
                _smallPicUrlString = [_smallPicUrlString stringByAppendingString:[NSString stringWithFormat:@",%@",_imageUrlArray[i]]];
                _bigPicUrlString = [_bigPicUrlString stringByAppendingString:[NSString stringWithFormat:@",%@",_bigImageUrlArray[i]]];
            }
        }
    }
    _smallPicUrlString = [_smallPicUrlString substringWithRange:NSMakeRange(1, _smallPicUrlString.length-1)];
    _bigPicUrlString = [_bigPicUrlString substringWithRange:NSMakeRange(1, _bigPicUrlString.length-1)];
//     NSLog(@"_memoString==%@",_memoString);
    _memoString = [_memoString substringWithRange:NSMakeRange(1, _memoString.length-1)];
//    NSLog(@"_smallPicUrlString==%@",_smallPicUrlString);
//    NSLog(@"_bigPicUrlString==%@",_bigPicUrlString);
...
-(NSInteger)getIndexAtList
{
    ...
    for (int i = 0 ; i < _industryNameArray.count; i++) {
        ...
        if ([model.name isEqualToString:self.model.tradename]) {
            selectedIndex = i;
            _selectListIndex = model.industryId ;
        }
    }
    NSLog(@"%@",_selectListIndex);
...
{
    [_imageUrlArray removeObjectAtIndex:btn.tag - KTag];
    [_bigImageUrlArray removeObjectAtIndex:btn.tag - KTag];
...
-(void)changeHeight
{
    ...
    }else if (_imageUrlArray.count >=3 && _imageUrlArray.count <= 6)
    {
...
{
    _bgView = [[UIView alloc]initWithFrame:CGRectMake(0, getNumWithScanf(12), SCREEN_WIDTH, getNumWithScanf(882))];
...
{
    if (indexPath.item == _imageUrlArray.count) {
    ...
    if (![_imageUrlArray[indexPath.item] isKindOfClass:[UIImage class]]) {
        [cell.imageView sd_setImageWithURL:[NSURL URLWithString:_imageUrlArray[indexPath.item]] placeholderImage:[UIImage imageNamed:@"loadFailed"] options:SDWebImageAllowInvalidSSLCertificates];
    }else
    {
        cell.imageView.image = _imageUrlArray[indexPath.item];
    }
//    NSLog(@"--url===%@",_imageUrlArray[indexPath.item]);
...
{
    if (indexPath.item == _imageUrlArray.count) {
        [self pushImagePickerControllerWithImagesCount:(6-_imageUrlArray.count)];
    }
}
...
//    _selectedAssert = [NSMutableArray arrayWithArray:assets];
    ...
    for (int i = 0; i < _selectedPhoto.count; i++) {
        [_imageUrlArray addObject:_selectedPhoto[i]];
        [_bigImageUrlArray addObject:_selectedPhoto[i]];
    }
