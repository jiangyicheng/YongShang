...
@interface EnterPriseCertificationViewController ()<UIScrollViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
*  企业认证接口
*/
-(void)submitTheCertificationWithNewsDetails:(NSString*)newsDetails
{
    [self showprogressHUD];
    [YSBLL enterPriseCertificationAddWithFlagName:_enterPriseNameTextField.text andNewsdetails:newsDetails andOperate:_validDateTextField.text andUserid:[UserModel shareInstanced].userID andResultBlock:^(CommonModel *model, NSError *error) {
...
//    if(![TSRegularExpressionUtil validateYear:_validDateTextField.text])
//    {
//        [self showMessage:@"请输入正确有效期"];
//        return;
//    }
//    CertificationINGViewController* cvc = [[CertificationINGViewController alloc]init];
//    [self.navigationController pushViewController:cvc animated:YES];
...
-(void)creatSubViews
{
    ...
    _licenseImageView = [[UIImageView alloc]initWithFrame:CGRectMake(getNumWithScanf(140), getNumWithScanf(180), getNumWithScanf(100), getNumWithScanf(100))];
    ...
    _licenseImageView.image = [UIImage imageNamed:@"upLoadPicture"];
