//
//  FriendEnterPriseDetailViewController.m
//  YongShang
//
...
@interface FriendEnterPriseDetailViewController ()<UIScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,SDPhotoBrowserDelegate>
{
...
@implementation FriendEnterPriseDetailViewController
...
{
    [YSBLL getEnterPriseResultWithFlagId:self.model.enterPriseId andCompanyid:[UserModel shareInstanced].companyId Block:^(EnterPriseDetailModel *model, NSError *error) {
...
-(void)passFriendShipApplyWithType:(NSString*)type
{
    ...
    [YSBLL getCommonWithUrl:[NSString stringWithFormat:@"appservice/friendsBusinessApply.do?createcompanyid=%@&friendid=%@&lxtype=%@&u_code=%@",[UserModel shareInstanced].companyId,self.model.enterPriseId,type,[UserModel shareInstanced].postName] andResultBlock:^(CommonModel *model, NSError *error) {
        if ([model.ecode isEqualToString:@"0"]) {
...
-(void)complaintEnterPriseClick
{
    ...
    rvc.beComplaintedCId = self.model.enterPriseId;
    rvc.complaintedUId = @"";
    rvc.complaintedCId = [UserModel shareInstanced].companyId;
    rvc.infomationSource = @"5";
    rvc.flagId = self.model.enterPriseId;
    rvc.context = self.model.mainbusiness ? self.model.mainbusiness : @"";
...
-(void)judge
{
    if ([self.model.type isEqualToString:@"11"]){
