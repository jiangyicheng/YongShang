//
//  RePublishSellAndBuyViewController.m
//  YongShang
//
...
@interface RePublishSellAndBuyViewController ()<UIScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIGestureRecognizerDelegate,JYCDropDownMenuDelegate,TZImagePickerControllerDelegate,UITextViewDelegate>
{
...
@implementation RePublishSellAndBuyViewController
...
-(void)uploadImage
{
    ...
    [YSBLL sellAndBuyUploadimagesWithflagId:self.flagId producturl1:_bigPicUrlString ? _bigPicUrlString : @"" producturl1s:_smallPicUrlString ? _smallPicUrlString : @"" memo:_memoString ? _memoString :@"" andResultBlock:^(GQAddSupplyAndDemandModel *model, NSError *error) {
        [self hiddenProgressHUD];
        if ([model.ecode isEqualToString:@"0"]) {
 ...
 *  供求信息重新发布
 */
-(void)RePublishMySupplyAndDemandWithType:(NSString*)type
{
    BOOL state ;
    ...
    [self showprogressHUD];
    [YSBLL revocationOrPublishTheSupplyAndDemandWithFlagId:self.flagId andCreatecid:[UserModel shareInstanced].userID andLxtype:[self judgeTheCategory] andContext:_contentTextField.text andTelphonestatus:telStatus andTradeId:[self judgeTheValid] imagesuccess:type andResultBlock:^(GQAddSupplyAndDemandModel *model, NSError *error) {
        if ([model.ecode isEqualToString:@"0"]) {
            NSLog(@"%@",model.sellbuyid);
...
-(void)loadEnterPriseDetailData
{
    ...
    if (_detailModel) {
        if (![_detailModel.sellbuyimage1 isEqualToString:@""]) {
            NSString* tempStr = [_detailModel.sellbuyimage1s substringWithRange:NSMakeRange(0, 1)];
            NSString* smallImageStr = _detailModel.sellbuyimage1s;
            NSString* bigImageStr = _detailModel.sellbuyimage1;
            //如果第一个字符是逗号的话就删除逗号
            ...
            [_imageUrlArray addObjectsFromArray:arrs];
...
-(void)PublishClick
{
    ...
    if (self.flagId) {
        if (_imageUrlArray.count > 0) {
            [self RePublishMySupplyAndDemandWithType:@"0"];
        }else{
            [self RePublishMySupplyAndDemandWithType:@"1"];
        }
    }
}
...
//    [_selectedPhoto addObjectsFromArray:photos];
    //    _selectedAssert = [NSMutableArray arrayWithArray:assets];
    
//    _imageUrlArray = [[NSMutableArray alloc]init];
    for (int i = 0; i < photos.count; i++) {
        [_imageUrlArray addObject:photos[i]];
        [_bigImageUrlArray addObject:photos[i]];
        
    }
//    [_imageUrlArray addObjectsFromArray:photos];
    [self changeHeight];
    [_collectionView reloadData];
}
